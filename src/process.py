...
def main(opt):
    ...
    diabetes_nan = diabetes_csv.dropna().drop_duplicates(subset= ['patient_nbr'], keep = 'first')

    # Drop unnecessary columns 
    diabetes_clean = diabetes_nan.drop(columns = ["encounter_id", "patient_nbr", "race", "weight", 
                                              "payer_code", "medical_specialty", "examide", "citoglipton"])
    # Drop unnecessary columns except race
    diabetes_with_race = diabetes_nan.drop(columns = ["encounter_id", "patient_nbr","weight", 
                                              "payer_code", "medical_specialty", "examide", "citoglipton"])
